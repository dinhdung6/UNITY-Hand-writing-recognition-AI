...
public class Writing : MonoBehaviour
{
    ...
    /// <summary>
    /// Amount of delay to auto-predict after a line has finished drawing. Insert negative values to turn off auto-prediction
    /// </summary>
    [Tooltip("Amount of delay to auto-predict after a line has finished drawing. Insert negative values to turn off auto-prediction")]
    public float predictDelay = 10.0f; // Set to 5 seconds
    ...
    protected virtual void Start()
    {
        ...
        _agent = GetComponent<HandwritingAgent>(); // Get reference to the HandwritingAgent
        _lastPredictIdx = 0;
    }
    ...
    protected virtual void Update()
    {
        ...
        {
            ClearAllLines(); // Clear lines when X is pressed
        }

        // Manual prediction
        if (Input.GetKeyDown(KeyCode.F))
        {
            PerformPrediction(); // Trigger prediction manually
        }
        ...
        if (predictDelay > 0 && _lastPredictIdx < _lineParent.childCount)
        {
            ...
            {
                PerformPrediction(); // Perform auto prediction
            }
        }
    }

    /// <summary>
    /// Performs prediction based on the current lines drawn
    /// </summary>
    private void PerformPrediction()
    {
        _agent.Predict(GetCurrentLinesData(_lastPredictIdx));
        _lastPredictIdx = _lineParent.childCount; // Update the last index
        _lastPredict = Time.time; // Reset the last prediction time
    }

    /// <summary>
    /// Clears all the drawn lines on the surface and resets the handwriting agent.
    /// </summary>
    public void ClearAllLines()
    {
        foreach (Transform line in _lineParent)
        {
            Destroy(line.gameObject); // Destroy each line renderer
        }
        _currentLine = null; // Reset the current line reference
        _lastPredict = Time.time; // Reset the last prediction time
        _lastPredictIdx = 0; // Reset last prediction index

        // Reset handwriting agent's state
        if (_agent != null)
        {
            _agent.ResetInput(); // Call reset method to clear input on HandwritingAgent
        }
    }
    ...
    protected void UpdateLine(Vector3 point, Vector3 normal, Vector3 up)
    {
        ...
        if (_currentLine == null)
        {
            ...
            _currentLine.transform.rotation = Quaternion.LookRotation(normal, up);
        ...
        if (_lastLinePlace + lineFidelity <= Time.time)
        {
            ...
            _currentLine.SetPosition(_currentLine.positionCount - 1,
                _currentLine.transform.InverseTransformPoint(point - normal * .01f));
    ...
    protected WritingAreaInfo GetCurrentLinesData(int startIdx)
    {
        ...
        areaInfo.size = Mathf.Max(sizeX, sizeY) * boundSizeFactor; // Largest size between x and y, also multiply by bound padding factor
    ...
    public void SimulateXKeyPress()
    {
        ClearAllLines(); // Call the same method used for X key press
    }
}
